...
```bash
As soon as the PR is created, the workflow started running.
```
...
```bash
cron expression for every monday at 9 AM is:-
'0 9 * * 1'
```
...
```bash
(YES THE WORKFLOW WORKED FINE AND THE MSG WAS PRINTED)
```
---
...
```bash
On including 2 OS, now a total of 6 jobs are running - 3 on 1st OS and 3 on 2nd OS.
```
